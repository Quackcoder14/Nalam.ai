    ...
    async function init() {
      ...
      try {
        const pos = await Promise.race([
          ...
          new Promise<never>((_, reject) => setTimeout(() => reject(new Error('Geolocation timeout')), 8000))
        ]);
        if (cancelled) return;
      ...
      try {
        ...
        const styleRes = await fetch(styleObj, { signal: controller.signal });
        ...
        console.warn('Failed to sanitize style JSON (CORS or timeout), falling back to URL directly', e);
      }
      if (cancelled) return;
      ...
      loadTimeout = setTimeout(() => {
        if (status !== 'ready' && !cancelled) {
          ...
          retryMap.on('error', (e: any) => console.warn('Retry map error:', e));
          mapRef.current = retryMap;
        }
      }, 12000);
