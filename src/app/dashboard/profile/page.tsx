    ...
    <div style={{ minHeight: '100dvh', background: 'var(--background)', paddingBottom: 'calc(6rem + env(safe-area-inset-bottom))' }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: '0.78rem', color: 'var(--charcoal)', margin: 0 }}>
            {t('profile.editDetailsDesc') || 'Update your personal health information'}
          </p>
        </div>
        {hasDraft && (
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '1rem 1rem 0' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          ...
          <ProfileField {...fieldProps('marital_status')} label={t('profile.maritalStatus') || 'Marital Status'} type="select"
            options={['Single', 'Married', 'Divorced', 'Widowed']} />
          <ProfileField {...fieldProps('occupation')} label={t('profile.occupation') || 'Occupation'} />
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.6rem' }}>
            ...
            <ProfileField {...fieldProps('district')} label={t('profile.district') || 'District'} />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.6rem' }}>
            <ProfileField {...fieldProps('state')} label={t('profile.state') || 'State'} />
            <ProfileField {...fieldProps('pincode')} label={t('profile.pincode') || 'Pincode'} type="tel" />
          </div>
        </div>

        {/* Emergency Contact */}
        <Section icon={Phone} title={t('profile.emergencyContact') || 'Emergency Contact'} color="#DC2626" />
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          <ProfileField {...fieldProps('emergency_name')} label={t('profile.emergencyName') || 'Name'} />
          <ProfileField {...fieldProps('emergency_relation')} label={t('profile.emergencyRelation') || 'Relationship'} />
          <ProfileField {...fieldProps('emergency_phone')} label={t('profile.emergencyPhone') || 'Phone'} type="tel" />
        </div>

        {/* Medical Info */}
        <Section icon={Heart} title={t('profile.medicalInfo') || 'Medical Information'} color="#DC2626" />
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          <ProfileField {...fieldProps('blood_type')} label={t('profile.bloodType') || 'Blood Type'} type="select"
            options={['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-']} />
          <ProfileField {...fieldProps('allergies')} label={t('profile.allergies') || 'Allergies'} />
          <ProfileField {...fieldProps('chronic_conditions')} label={t('profile.chronicConditions') || 'Chronic Conditions'} />
          <ProfileField {...fieldProps('current_medications')} label={t('profile.currentMedications') || 'Current Medications'} />
          <ProfileField {...fieldProps('past_surgeries')} label={t('profile.pastSurgeries') || 'Past Surgeries'} />
          <ProfileField {...fieldProps('family_history')} label={t('profile.familyHistory') || 'Family Medical History'} />
        </div>

        {/* Insurance */}
        <Section icon={Shield} title={t('profile.insurance') || 'Insurance'} color="#7C3AED" />
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          <ProfileField {...fieldProps('insurance_provider')} label={t('profile.insuranceProvider') || 'Insurance Provider'} />
          <ProfileField {...fieldProps('insurance_policy')} label={t('profile.insurancePolicy') || 'Policy Number'} />
        </div>
